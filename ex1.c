...
void DFS(GPH *g,STK *s,int v_nr)
{
    ...
    while (aux != NULL)
    {
        ...
        if (g->vis[con_ver]==0) DFS(g,s,con_ver);
        aux=aux->next;
    }
}
...
void canbe(GPH *g, int nrv, STK *s1, STK *s2, int ans)// 0 sau 1 daca poate fi sau nu ajuns
{
...
int main()
{
    ...
    canbe((GPH*)&g, nrv, &s1, *(long long unsigned*)sizeof(s2),ans);
}
